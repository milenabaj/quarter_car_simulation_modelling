...
if __name__ == "__main__":
    ...
    parser.add_argument('--w_min', default = 0, type=int,
                        help = 'Defect width minimum. Use 0 for no selection.') 
    parser.add_argument('--w_max', default =  np.inf, type=int,
                        help = 'Defect width maximum. Use np.inf for no selection.') 
    parser.add_argument('--h_min', default = -np.inf, type=int,
                        help = 'Defect height minimum. Use -np.inf for no selection') 
    parser.add_argument('--h_max', default =  np.inf, type=int,
                        help = 'Defect height maximum. Use np.inf for no selection') 
    ...
    parser.add_argument('--attn', default = 'general',
                        help = 'Attention type to use in decoder. Choose between dot and general.') 
    # Directories
    parser.add_argument('--input_dir', help = 'Input directory containing train/valid/test subdirectories with prepared data split into windows.')
    parser.add_argument('--out_dir_base', default = 'results',
                        help='Output directory for results. A new directory with information about the model will be created here.')
    ...
    if args.model_type not in ['lstm_encdec', 'lstm_encdec_with_attn']:
        sys.exit('Unknown model passed. Choose beteen lstm_encdec and lstm_encdec_with_attn.')  
    ...
    decrease_teacher_forcing_ratio = True #decrease teacher_forcing_ration by 0.02 in every epoch
    ...
    log.info('Number of train samples: {0}'.format(sum(train_dataloader.dataset.cummulative_sizes)))
    log.info('Number of valid samples: {0}'.format(sum(valid_dataloader.dataset.cummulative_sizes)))
