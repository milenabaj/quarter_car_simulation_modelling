...
class lstm_encoder(nn.Module):
    ...
    def forward(self, x_input):
        ...
        # Concatenate bidirectional states (forward+backward) from the last encoder layer to prepare for unidirectional decoder
...
class lstm_decoder(nn.Module):
    ...
    def __init__(self, input_size = 32, hidden_size = 64, output_size = 1, num_layers = 1, device = 'cuda', attn='general'):
        ...
        if self.attn =='general':
            self.attention_layer = nn.Linear(self.hidden_size, self.hidden_size, bias=True)
    ...
    def forward(self, x_input, hidden_states, encoder_output, t):
        ...
        # Propagate through LSTM decoder layer
        ...
        # In general attention, decoder hidden state is passed through the linear layers to introduce the weight matrix             
